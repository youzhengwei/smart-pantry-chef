// Test the improved substring matching logic
...
function testMatching(query, productTexts) {
  ...
  const matched = queryWords.length === 0 ? [] : cleanedProductTexts.filter((productName, idx) => {
    ...
    const isMatch = queryWords.every(queryWord => {
      return productNorm.includes(queryWord);
    });
...
// Test Case 3: Searching for "chocolate"
...
testMatching('chocolate', productsForChocolate);
